...
public class DiscordInstanceImpl implements DiscordInstance {
	...
	public synchronized SubmitResultVO submitTask(Task task, Callable<Message<Void>> discordSubmit) {
		...
		try {
			...
			return SubmitResultVO.fail(ReturnCode.QUEUE_REJECTED, "The queue is full, please try again later")
					.setProperty(Constants.TASK_PROPERTY_DISCORD_INSTANCE_ID, this.getInstanceId());
		} catch (Exception e) {
			log.error("submit task error", e);
			return SubmitResultVO.fail(ReturnCode.FAILURE, "Submission failed, system exception")
					.setProperty(Constants.TASK_PROPERTY_DISCORD_INSTANCE_ID, this.getInstanceId());
		}
		if (currentWaitNumbers == 0) {
			return SubmitResultVO.of(ReturnCode.SUCCESS, "Submitted successfully", task.getId())
					.setProperty(Constants.TASK_PROPERTY_DISCORD_INSTANCE_ID, this.getInstanceId());
		} else {
			return SubmitResultVO.of(ReturnCode.IN_QUEUE, "In line，there is " + currentWaitNumbers + " more tasks ahead", task.getId())
	...
	private void executeTask(Task task, Callable<Message<Void>> discordSubmit) {
		...
		try {
			...
			task.fail("Execution error, system exception");
			saveAndNotify(task);
		} finally {
			this.runningTasks.remove(task);
			this.queueTasks.remove(task);
